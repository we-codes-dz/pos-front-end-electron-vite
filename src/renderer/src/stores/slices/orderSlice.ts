...
import { generatePendingOrderId, getAddOnsFromCurrentOrders } from '@renderer/utils/helper'
...
export const createOrderSlice: StateCreator<OrderSlice> = (set) => {
  ...
  return {
    ...
    addProductToPendingOrders: (order: TOrder) =>
      set((state: OrderSlice) => {
        const { sliders } = state
        const slideId = `slide_${++slideIdCounter}`
        const pendingOrderId = generatePendingOrderId(
        ...
        const updatedSliders = {
          ...sliders,
          [slideId]: [...(sliders[slideId] || []), order]
        }

        // Add orderItemsCarousel property to each order
        order.orderItemsCarousel = order.items.map((item) => ({
          ...item,
          orderId: order.id,
          slideId
        }))
